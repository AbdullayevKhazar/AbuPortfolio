...
const DEFAULT_FONT = '30px "Camingo Code"';
const DEFAULT_FONT_URL = '/fonts/CamingoCode-Regular.ttf';
...
async function resolveFont(font: string, fontUrl?: string): Promise<string> {
  // Load the bundled regular font when the caller relies on the default.
  ...
  try {
    ...
    const prefix = sizeMatch ? sizeMatch[1].trim() : '30px';
...
class Title {
  ...
  constructor({ gl, plane, renderer, text, textColor = '#545050', font = DEFAULT_FONT }: TitleProps) {
    autoBind(this);
    this.gl = gl;
    this.plane = plane;
